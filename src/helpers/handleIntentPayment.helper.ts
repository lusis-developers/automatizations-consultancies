...
export async function handleIntentPayment(body: IntentPaymentBody, res: Response): Promise<void> {
  try {
    ...
    try {
      if (isFirstPayment) {
        ...
        console.log('[Webhook - Email de Confirmación Enviado]', `Cliente: ${cliente.name}`)
      }
    } catch (emailError) {
